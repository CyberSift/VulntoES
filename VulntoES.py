    ...
    def toES(self):
        ...
        for rh in report:
            ...
            for tag in rh.getchildren():
                ...
                if tag.tag == 'HostProperties':
                    for child in tag.getchildren():
                        if child.attrib['name'] == 'HOST_END':
                            ...
                            host_item['time'] = datetime.strftime(host_item['time'], '%s') + '000'
                ...
                self.es.index(index=self.index_name, doc_type="vuln",
                              body=json.dumps(dict(host_item.items() + dict_item.items())))
...
class NmapES:
    ...
    def toES(self):
        ...
        for h in self.root.iter('host'):
            ...
            if h.tag == 'host':
                if h.attrib['endtime']:
                    dict_item['time'] = time.strftime('%s', time.gmtime(float(h.attrib['endtime']))) + '000'
...
class OpenVasES:
    ...
    def __init__(self, input_file, es_ip, index_name):
        ...
        self.tree = self.__importXML()
        self.root = self.tree.getroot()
        self.creation_time = self.root.find('creation_time')
        ...
        for result in self.root.iter('result'):
            ...
            if issueDict:
                if self.creation_time is not None:
                    issueDict['time'] = str(int(time.mktime(datetime.strptime(self.creation_time.text, "%Y-%m-%dT%H:%M:%SZ").timetuple())))+'000'
...
def main(report_type=None, index_name=None, in_file=None, es_ip=None, static_fields=None):
...
try:
    with open('/ml_infra/openvas/processed_reports') as f:
        processed_reports = pickle.load(f)
except:
    processed_reports = []

s = check_output(["openvas-cli", "-h", openvas_ip, "-u", openvas_admin, "-w", openvas_password, "-G"])

for task in s.split('\n'):
    if task is not '':
        taskID = task.split('  ')[0]
        t = check_output(["openvas-cli", "-h", openvas_ip, "-u", openvas_admin, "-w", openvas_password, "-G", taskID])
        for report in t.split('\n'):
            if report.startswith('  '):
                reportID = report.split('  ')[1]
                if not (reportID in processed_reports):
                    processed_reports.append(reportID)
                    call("openvas-cli -u "+openvas_admin+" -w "+openvas_password+" -h"+ openvas_ip +"-R "+reportID+" > /tmp/test.xml", shell=True)
                    main(report_type='openvas', index_name='vuln', in_file='/tmp/test.xml', es_ip=elastic_ip)

with open('/ml_infra/openvas/processed_reports', 'wb') as f:
    pickle.dump(processed_reports,f)
